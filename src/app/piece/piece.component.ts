import { Component, ElementRef, Input, OnInit, Sanitizer, SecurityContext, ViewChild } from '@angular/core';
...
export class PieceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.iStateService.iFreePlace$.subscribe(() => this.iCanMove = this.iStateService.canMove(this.iPiece.x, this.iPiece.y));
  ...
  imgLoad(img: HTMLImageElement) {
    if (img?.getClientRects().length > 0) {
      ...
      this.iStateService.iImageSize$.next({ width: `${width}`, height: `${height}` });
