...
export class PuzzleComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.iStateService.iDimension$.subscribe(
      (mDimension) => (this.iParts = mDimension)
    );
    this.iStateService.iPieces$.subscribe(
      (mPieces) => (this.iPieces = mPieces)
    );
    ...
    if (!this.iAutoPlaying) {
      ...
      this.iSubscription$ = interval(200).subscribe(() => {
        if (this.iHaveWon) {
          ...
          const mAvailableMoves = this.iPieces.filter((p) =>
            this.iStateService.canMove(p.x, p.y)
          );
          const mRandomPiece =
            mAvailableMoves[Math.floor(Math.random() * mAvailableMoves.length)];
